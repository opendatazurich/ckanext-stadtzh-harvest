...
class TestStadtzhHarvester(object):
    ...
    def test_load_metadata_from_path(self):
        ...
        test_meta_xml_path = os.path.join(
            data_path, dataset_folder, "meta.xml"
        )
        ...
        assert metadata["title"] == u"Administrative Einteilungen Stadt Zürich"
        assert metadata["license_id"] == u"cc-by"
    ...
    def test_load_metadata_no_license(self):
        ...
        assert metadata["datasetFolder"] == dataset_folder
        assert metadata["datasetID"] == dataset_folder
        assert metadata["license_id"] == u"cc-zero"
    ...
    def test_load_metadata_resource_descriptions(self):
        ...
        assert (
            "test.json" in metadata["resource_metadata"]
        ), "test.json is not defined"
        assert (
    ...
    def test_load_metadata_attributes(self):
        ...
        anzahl = attributes[8]
        assert anzahl[0] == u"Gezählte Velofahrten"
        assert (
            anzahl[1]
            == u"Anzahl Velos pro Stunde an der jeweiligen Messstelle"
        )
    ...
    def test_load_metadata_groups_and_tags(self):
        ...
        check_group(
            metadata["groups"][2]["name"], "bevolkerung", u"Bevölkerung"
        )
...
class TestStadtzhHarvestFunctional(FunctionalHarvestTest):
    def test_harvest_create_test_dropzone(self):
        ...
        assert results[0]["name"] == u"test_dataset"
        assert (
            results[0]["title"] == u"Administrative Einteilungen Stadt Zürich"
        )
        assert results[0]["license_id"] == u"cc-by"
        assert results[0]["updateInterval"][0] == u"woechentlich"
        assert results[0]["dataType"][0] == u"Einzeldaten"
    ...
    def test_geo_with_resources(self):
        ...
        assert result["title"] == u"Administrative Einteilungen Stadt Zürich"
        assert result["license_id"] == u"cc-zero"
        assert len(result["resources"]) == 4

        test_json = next(
            r for r in result["resources"] if r["name"] == "test.json"
        )
        assert test_json["description"] == u"This is a test description"

        test_csv = next(
            r for r in result["resources"] if r["name"] == "test.csv"
        )
        assert test_csv["description"] == u""

        wms = next(
            r for r in result["resources"] if r["name"] == "Web Map Service"
        )
        assert wms["description"] == u""

        wfs = next(
            r
            for r in result["resources"]
            if r["name"] == "Web Feature Service"
        )
        assert wfs["description"] == u"Dies ist eine Spezial-Beschreibung"
    ...
    def test_harvest_create_with_dataset_prefix(self):
        ...
        results = self._test_harvest_create(1, config=test_config)["results"]
        assert len(results) == 1
        assert results[0]["name"] == u"testprefix-test_dataset"
        assert (
            results[0]["title"] == u"Administrative Einteilungen Stadt Zürich"
        )
        assert results[0]["license_id"] == u"cc-by"
    ...
    # TODO: This test not only fails, but causes all tests run after it to
    #  fail, and I can't figure out why.
    ...
    #
    #     fq = "+type:dataset harvest_source_id:{0}".format(
    #         harvest_source["id"])
    #     results = helpers.call_action("package_search", {}, fq=fq)
    ...
    #     try:
    #         assert next(
    #             r for r in result["resources"] if r["name"] == "fail.json"
    #         )
    #         assert next(
    #             r
    #             for r in result["resources"]
    #             if r["name"] == "Web Map Service"
    #         )
    #     except StopIteration:
    #         raise AssertionError(
    #             "Resources fail.json/Web Map Service not found"
    #         )
    #
    ...
    def test_delete_dataset(self):
        ...
        harvest_source = self._update_harvest_source(
    ...
    def test_harvest_update_dwh(self):
        ...
        meta_xml_path = os.path.join(
            self.temp_dir, "DWH", "nachnamen_2014", "meta.xml"
        )
    ...
    def _test_harvest_update(
        ...
        **kwargs
        ...
        shutil.copytree(
            dataset_path, os.path.join(dropzone_path, "test_dataset")
        )
    ...
    def test_harvest_update_resources_geo(self):
        ...
        assert (
            test_json["description"] == "This is a test description (updated)"
        )

    def _test_harvest_update_resource(
        self, num_objects, meta_xml_path, **kwargs
    ):
        harvest_source = self._create_harvest_source(**kwargs)
