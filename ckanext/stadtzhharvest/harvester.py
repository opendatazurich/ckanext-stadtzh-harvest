...
class StadtzhHarvester(HarvesterBase):
    ...
    def _check_for_deleted_datasets(self, harvest_job,
                                    gathered_dataset_nameids):
        existing_packages_nameids = \
            self._get_existing_packages_names(harvest_job)
        delete_nameids = list(set(existing_packages_nameids) -
                              set(gathered_dataset_nameids))
        # gather delete harvest ids
        delete_ids = []
        for package_nameid in delete_nameids:
            log.debug('Dataset `%s` has been deleted' % package_nameid)
            ...
            self._create_notification_for_deleted_dataset(package_nameid)
            if self.config['delete_missing_datasets']:
                log.info('Add `%s` for deletion', package_nameid)
                id = self._save_harvest_object(
                    {
                        'datasetID': package_nameid,
